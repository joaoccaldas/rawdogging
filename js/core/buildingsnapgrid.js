...
export class BuildingSnapGridSystem {
    ...
    updateGhostPosition() {
        ...
        // Use unified interaction utils selection
        // This ensures the ghost follows the mouse cursor exactly like mining/placing
        const hit = InteractionUtils.getSelection(this.game, 6); // slightly longer range for building?

        let targetX, targetY, targetZ;

        if (hit) {
            // If we hit something, try to place against it
            const placePos = InteractionUtils.getPlacementPosition(this.game, hit);
            if (placePos) {
                targetX = placePos.x;
                targetY = placePos.y;
                targetZ = placePos.z;
            } else {
                // Hit something but can't place? e.g. inside player?
                // Fallback to hit pos
                targetX = hit.x;
                targetY = hit.y;
                targetZ = hit.z;
            ...
            // No hit, project ray to ground plane?
            // Fallback to "in front of player" if mouse is skyward, 
            // OR reuse the Player.js fallback logic if possible.
            // For now, let's stick to the previous "Look Direction" fallback if no mouse hit,
            // BUT InteractionUtils is mouse based.

            // If InteractionUtils failed, it means we pointed at sky/void.
            // Let's float it in front of player.
            const lookDir = player.lookDirection || { x: 1, y: 0 };
            const placeDistance = 3;
            targetX = player.x + lookDir.x * placeDistance;
            targetY = player.y + lookDir.y * placeDistance;
            targetZ = player.z;
        }
